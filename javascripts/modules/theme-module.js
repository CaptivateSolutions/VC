AppName.Modules.ThemeModule = (function () {
    ...
    var _roomSelectSubmit = function () {
        $(document).ready(function () {
            if ($('main').hasClass('room-selection')) {
                $(".room-sr-holder input[type='checkbox']").change(function () {
                    if ($(this).is(":checked")) {
                      console.log('CHECKED!')
                      $(".js-form-submit").submit();
                    }
                });
            }
        });
    ...
    // Function responsible for passing date and time
    var _passDateTime = function () {
      // On load
      $(document).ready(function(){
        // Check if a Check-in Date is already selected and set its value. If it doesn't exist, place an empty value on input field until user selects a date.
        if (localStorage.checkInDate) {
            $('.js-checkindate-home, .js-checkindate-selection').val(localStorage.checkInDate);
        } else {
          localStorage.setItem('checkInDate', '');
        }

        // Check if a Check-out Date is already selected and set its value. If it doesn't exist, place an empty value on input field until user selects a date.
        if (localStorage.checkOutDate) {
            $('.js-checkoutdate-home, .js-checkoutdate-selection').val(localStorage.checkOutDate);
        } else {
          localStorage.setItem('checkOutDate', '');
        }

        // Check if a Check-in Time is already selected and set its value. If it doesn't exist, place an empty value on input field until user selects a date.
        if (localStorage.checkInTime) {
            $('.js-checkintime-home, .js-checkintime-selection').val(localStorage.checkInTime);
        } else {
          localStorage.setItem('checkInTime', '');
        }
      });

      // Changing Check-in Date
      $('.js-checkindate-home').blur(function(){
        var checkInDateVar = $(this).val();
        localStorage.setItem('checkInDate', checkInDateVar);
      });

      // Changing Check-out Date
      $('.js-checkoutdate-home').blur(function(){
        var checkOutDateVar = $(this).val();
        localStorage.setItem('checkOutDate', checkOutDateVar);
      });

      // Changing Check-in Time
      $('.js-checkintime-home').blur(function(){
        var checkInTimeVar = $(this).val();
        localStorage.setItem('checkInTime', checkInTimeVar);
      });
    };
    ...
    var init = function () {
        ...
        _openFilter();
        _mobileArrangement();
        // _roomSelect(); disable redirection upon clicking on a room
        _roomSelectSubmit();
        _socialHover();
        _passDateTime(); // Initialize passing of date and time
    };
