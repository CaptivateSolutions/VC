...
*/	
	...
	//20170505
	//tama compute 1 day lang difference
	if ($no_of_days == 0) 
	{
		...
		<?php print($rldesc); ?>
                                        ...
                                        <div class="col-xs-12 col-sm-6 less-padding-right">
                                            <div class="confirm-customer-details">
                                                <p class="title">Guest Details</p>
                                                <p>Guest Name : <strong><?php print($fname . ' ' . $lname); ?></strong></p>
...
<?php
	...
	//20170505				
	$tlname = str_replace('"', '&quot;', $tlname); 
	$tlname = str_replace('\'', '&acute;', $tlname); 
	$tlname = htmlspecialchars($tlname, ENT_QUOTES);
	
	$tlemail = $email;

	//20170505				
	$tlemail = str_replace('"', '&quot;', $tlemail); 
	$tlemail = str_replace('\'', '&acute;', $tlemail); 
	$tlemail = htmlspecialchars($tlemail, ENT_QUOTES);

	$tlphone = $contact;

	//20170505				
	$tlphone = str_replace('"', '&quot;', $tlphone); 
	$tlphone = str_replace('\'', '&acute;', $tlphone); 
	$tlphone = htmlspecialchars($tlphone, ENT_QUOTES);

	$tlmobile = $contact;
	
	//20170505				
	$tlmobile = str_replace('"', '&quot;', $tlmobile); 
	$tlmobile = str_replace('\'', '&acute;', $tlmobile); 
	$tlmobile = htmlspecialchars($tlmobile, ENT_QUOTES);
	...
	// echo '<br> _SESSION tlid : ' . $_SESSION['tlid'];
	// echo '<br> _SESSION tldocno : ' . $_SESSION['tldocno'];
	// echo '<br> _SESSION tlsession : ' . $_SESSION['tlsession'];
	
	if (($_SESSION['tlid'] != '') && ($rlid != 0))
	{
		//rooom info
		$trid = 0;
		$trdate = $globaldate;
		...
		$trname = str_replace('\'', '&acute;', $trname); 
		$trname = htmlspecialchars($trname, ENT_QUOTES);
		...
		//20170505				
		$rlname = str_replace('"', '&quot;', $rlname); 
		$rlname = str_replace('\'', '&acute;', $rlname); 
	...
	{	
		...
		if ($_SESSION['promo_valid'] == 'VALID')
		{
			...
			//20170505				
			$tpcode = str_replace('"', '&quot;', $tpcode); 
			$tpcode = str_replace('\'', '&acute;', $tpcode); 
			...
			$tpname = str_replace('\'', '&acute;', $tpname); 
	...
	if ($_SESSION['tlid'] != '')
	{
		...
		while ($mycounter <= 30)
		{
			...
			{ 
				...
				//20170505				
				$tfname = str_replace('"', '&quot;', $tfname); 
				$tfname = str_replace('\'', '&acute;', $tfname); 
	...
	if ($_SESSION['tlid'] != '')
	{
		...
		while ($mycounter <= 30)
		{
			...
			{ 
				...
				//20170505				
				$taname = str_replace('"', '&quot;', $taname); 
				$taname = str_replace('\'', '&acute;', $taname); 
...
//////////////// PAYNAMICS ////////////////////////
	...
	{
		//20170505
		//send amount per piece
		$tramt = $tramt / $trdays;
		$tramt2 = number_format($tramt,2);
		$tramt2 = str_replace(",","", $tramt2);
		$strxml = $strxml . "<Items><itemname>" . $trname . "</itemname><quantity>" . $trdays . "</quantity><amount>" . $tramt2 . "</amount></Items>";
	} 
	...
	if ($tpamt > 0) 
	{
		...
		$strxml = $strxml . "<Items><itemname>" . $tpname . "</itemname><quantity>" . $tpqty2 . "</quantity><amount>" . $tpamt2 . "</amount></Items>"; 
	}
	...
	while ($mycounter <= 30)
	{
		...
		{ 
			...
			$strxml = $strxml . "<Items><itemname>" . $addon_name . "</itemname><quantity>" . $addon_qty . "</quantity><amount>" . $myamount2 . "</amount></Items>"; 
		}		
	...
	while ($mycounter <= 30)
	{
		...
		{ 
			...
			$myamount2 = number_format($myamount,2);
			$myamount2 = str_replace(",","", $myamount2);	
			$strxml = $strxml . "<Items><itemname>" . $food_name . "</itemname><quantity>" . $food_qty . "</quantity><amount>" . $myamount2 . "</amount></Items>"; 
		}		
...
<?php	
	...
	{
?> 	
<form id = "paypal_checkout" action = "index.php" method = "post">
	<input id = "ppcheckoutbtn" value = "BACK TO HOME PAGE"  class="btn next-step gray-bttn" type = "submit">
</form>	
<?php											
	}
